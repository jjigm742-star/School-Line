...
const SPEED_TIERS = [3.2, 4.0, 5.0, 6.0, 7.2];
...
function nextSpeedTier(speed) {
  for (let i = 0; i < SPEED_TIERS.length; i++) {
    if (Math.abs(SPEED_TIERS[i] - speed) < 0.001) return SPEED_TIERS[Math.min(i + 1, SPEED_TIERS.length - 1)];
  }
  return speed * 1.2;
}
...
function publicCharacterDefs() {
  ...
  for (const [id, c] of Object.entries(CHARACTERS)) {
    out[id] = { name: c.name, role: c.role, hp: c.hp, speed: c.speed, fireRate: c.fireRate, projectileType: c.projectileType };
  }
  return out;
}
...
function startMatch(room) {
  ...
  for (const p of room.players.values()) {
    ...
    Object.assign(p, {
      ...
      nextFireAt: 0, burnUntil: 0, burnDps: 0, tailwindUntil: 0
    });
...
function die(room, player, now) {
  ...
  player.burnUntil = 0; player.burnDps = 0; player.tailwindUntil = 0;
...
function updateRoom(room, dt, now) {
  ...
  for (const player of room.players.values()) {
    ...
    if (ml > 0) { mx /= ml; my /= ml; }
    const speed = player.tailwindUntil > now ? nextSpeedTier(def.speed) : def.speed;
    movePlayer(player, mx * speed * dt, my * speed * dt, def.radius);

    if (player.input.fire && now >= player.nextFireAt) {
      spawnProjectile(room, player, def, now);
      player.nextFireAt = now + 1000 / def.fireRate;
    }
  }
...
function snapshot(room) {
  ...
  return {
    ...
    players: [...room.players.values()].map(p => ({
      ...
      burning: p.burnUntil > now, tailwind: p.tailwindUntil > now
    })),
